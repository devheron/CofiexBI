...
- [ ] Padronizar categorias de `de_tipo_operacao`.
- [ ] Padronizar categorias de `de_fase`.
- [ ] Padronizar categorias de `de_esfera`.
- [ ] Padronizar categorias de `nm_regiao`.
- [ ] Padronizar siglas em `sg_uf` e `sg_fonte`.
- [ ] Tratar campos textuais vazios como nulos.
...
- [ ] Criar flags de repeticao de `cd_pleito` e `nu_operacao`.
- [ ] Criar flags de ausencia para campos-chave.
...
- [ ] Criar flag `cronologia_invalida`.
- [ ] Criar relatorio de qualidade por coluna.
- [ ] Comparar linhas antes e depois do tratamento.
- [ ] Validar ranges plausiveis de datas.
- [ ] Validar valores negativos, nulos e zeros em campos financeiros.

### Carga

- [ ] Exportar base tratada para `dados_tratados/`.
- [ ] Salvar CSV com encoding `utf-8-sig`.
- [ ] Testar leitura do CSV tratado.
...
### Estado atual do `df_tratado`

Apos as limpezas, o `df_tratado` possui:

- 3.548 linhas (mesmo total da base bruta, nenhuma linha removida).
- 25 colunas originais preservadas.
- 5 colunas padronizadas criadas: `de_tipo_operacao_padronizado`, `de_fase_padronizado`, `de_esfera_padronizado`, `nm_regiao_padronizado`, `sg_uf_padronizado`, `sg_fonte_padronizado`.
- 5 flags de ausencia criadas: `flag_sem_fonte`, `flag_sem_financiamento`, `flag_sem_contrapartida`, `flag_sem_primeira_cofiex`, `flag_sem_assinatura`.

### Proximas etapas pendentes na limpeza

- Converter colunas de data com `pd.to_datetime(..., errors="coerce")`.
- Garantir valores monetarios numericos.
